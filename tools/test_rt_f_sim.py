...
parser.add_argument('--datasetroot', default='./testing_dataset/UAV123_20L/',type=str,
        help='datasetsroot')
parser.add_argument('--sim_info', default='./testing_dataset/DTB70_SiamMask_sim.pkl', help='dataset_sim info')
...
parser.add_argument('--config', default='experiments/siammask_r50_l3/pre_kf_config.yaml', type=str,
        help='config file')
...
def main():
    ...
    for v_idx, video in enumerate(dataset):
        ...
        # result path
        o_path=os.path.join('results_rt_raw', args.dataset, 'Raw_pred_sim', model_name)
        ...
        while 1:
            ...
            if fidx == 0:
                ...
                # at least tracker should have 2 results for prediction
                ...
                # How about we fix?
                # pred_start_delta = fidx - last_fidx
                # pred_end = min(2*(fidx - last_fidx) + args.eta, predictor.max_target)
                condition = len(tracker.traject['fidx'])>1 if cfg.PRED.TYPE=='KF' else (len(tracker.traject['fidx'])>1 and (fidx - last_fidx)<cfg.TRAIN.PRE_TARGET)
                if condition:
